...
function BusinessObject(ses, name, instance) {
    ...
    this.count = function (filters, opts) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            self.session.req.call(self.session, self.path + '&action=count', getReqParams(self.filters), function (res, status) {
                var r = self.session.parse(res, status);
    ...
    this.search = function (filters, opts) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (opts.visible === true)
                p += '&_visible=true';
            self.filters = filters || {};
            self.session.req.call(self.session, self.path + '&action=search' + p, getReqParams(self.filters), function (res, status) {
                var r = self.session.parse(res, status);
    ...
    this.create = function (item, opts) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var p = getReqOptions(opts);
            self.session.req.call(self.session, self.path + '&action=create' + p, getReqParams(self.item), function (res, status) {
                var r = self.session.parse(res, status);
    ...
    this.update = function (item, opts) {
        ...
        return new Promise(function (resolve, reject) {
            if (item)
                self.item = item;
            var p = getReqOptions(opts);
            self.session.req.call(self.session, self.path + '&action=update' + p, getReqParams(self.item), function (res, status) {
                var r = self.session.parse(res, status);
    ...
    this.action = function (action, rowId, opts) {
        ...
        opts = opts || {};
        return new Promise(function (resolve, reject) {
            self.session.req.call(self.session, self.path + '&action=' + encodeURIComponent(action) + (rowId ? '&' + self.getRowIdFieldName() + '=' + encodeURIComponent(rowId) : ''), getReqParams(opts.parameters), function (res, status) {
                var r = self.session.parse(res, status);
    ...
    this.crosstab = function (crosstab, opts) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            self.session.req.call(self.session, self.path + '&action=crosstab&crosstab=' + encodeURIComponent(crosstab), getReqParams(self.filters), function (res, status) {
                var r = self.session.parse(res, status);
    ...
    this.setParameter = function (param, value, opts) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (value)
                p.value = value;
            self.session.req.call(self.session, self.path + '&action=setparameter', getReqParams(p), function (res, status) {
                var r = self.session.parse(res, status);
    ...
    this.getParameter = function (param, opts) {
        ...
        return new Promise(function (resolve, reject) {
            var p = { name: param };
            self.session.req.call(self.session, self.path + '&action=getparameter', getReqParams(p), function (res, status) {
                var r = self.session.parse(res, status);
